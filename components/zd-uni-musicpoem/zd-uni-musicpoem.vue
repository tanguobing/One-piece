<template>
	<view class="action" name="zdUniMusicpoem">
		<!-- <iframe id='goodsDescription' :src="music_words" frameborder="no" border="0" marginwidth="0" marginheight="0" scrolling="yes" ></iframe> -->
	...
	export default {
		...
		data() {
			return {
				...
				isPlayEnd: false,
				urls:audioUrl,
				music_words:"../../static/lovemeorhim-Ruy.lrc",
				currentTime: 0, //当前进度
			...
			getLyric(music_words) {
			   uni.request({
				...
				header:{'content-type': 'text/html'},
				   ...
				   // console.log(music_words)
				    // this.num = this.splitLyric(music_words);
					uni.setStorage({
						key : "storage_token",
						data: music_words,
					})
					this.lrcsetStorage = uni.getStorageSync("storage_token");
					this.num = this.splitLyric(this.lrcsetStorage);
					console.log(this.lrcsetStorage);
			   })
			},
			splitLyric(data) {
